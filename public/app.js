...
['qStore','qFocus','qCategory','qPriority','qResolved','qAging','qAck','obBy'].forEach(function(id){
  $(id).addEventListener('change', renderIssues);
});
$('qSearch').addEventListener('input', renderIssues);
$('qReset').addEventListener('click', function(){
  ['qArea','qStore','qFocus','qCategory','qPriority','qResolved','qAging','qAck'].forEach(function(id){
    $(id).value = '';
  });
  $('qSearch').value = '';
  buildIssueStoreFilter();
